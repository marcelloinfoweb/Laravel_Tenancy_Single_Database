        ...
        <div class="flex justify-between items-center">
            ...
            <div class="flex">
                <a href="{{route('cart.index', [request('subdomain')])}}" class="relative mr-8">
                    ...
                    @if(session()->has('cart'))
                        <span
                            class="absolute bg-red-500 text-white rounded-full p-1 right-0 top-1">{{count(session()->get('cart'))}}</span>
                    @endif
                </a>
        ...
        <div class="grid grid-cols-3 gap-1 mt-20">
            @forelse($store->products as $product)
                <div class="p-2 border border-gray-200 mb-8">
                    ...
                    <h3 class="font-extrabold text-2xl text-red-500">
                        R$ {{number_format($product->price, 2, ',', '.')}}</h3>

                    <a href="{{route('cart.add', [request('subdomain'), 'product' => $product->slug])}}"
                       class="underline text-blue-500">Comprar</a>
                </div>
            @empty
